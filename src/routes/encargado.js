...
router.post("/fAulaInsertar", function (req, res, next) {
  ...
  encargadoModel
    ...
    .then(
      encargadoModel.insertarListaAula(user, id).then((idAula) => {
...
router.post("/fAulaActualizar/", function (req, res, next) {
  ...
  encargadoModel
    ...
    .then(
      encargadoModel.insertarListaAula2(user, id).then((idAula) => {
        res.redirect("/encargado/fAula");
      })
    )
    .catch((err) => {
...
router.get("/fCarreraAgregar", checkNotAuthenticated, function (
  req,
  res,
  next
) {
...
router.post("/fCarreraInsertar", function (req, res, next) {
  ...
  encargadoModel
    ...
    .then(
      encargadoModel
        .insertarListaCarrera(user, codigoc, planc)
        .then((idAula) => {
...
router.post("/fCarreraActualizar/", function (req, res, next) {
  ...
  encargadoModel
    ...
    .then(
      encargadoModel
        .insertarListaCarrera2(user, codigoc, planc, estado)
        .then((idAula) => {
          res.redirect("/encargado/fCarrera");
        })
    )
...
router.get("/fDocenteAgregar", checkNotAuthenticated, function (
  req,
  res,
  next
) {
...
router.post("/fDocenteInsertar", function (req, res, next) {
  ...
  encargadoModel
    ...
    .then(
      encargadoModel.insertarListaDocente(user, codigod).then((idDocente) => {
...
router.post("/fDocenteActualizar/", function (req, res, next) {
  ...
  encargadoModel
    ...
    .then(
      encargadoModel
        .insertarListaDocente2(user, codigod, estado)
        .then((idDocente) => {
          res.redirect("/encargado/fDocente");
        })
    )
...
router.get("/fMateriaAgregar", checkNotAuthenticated, function (
  req,
  res,
  next
) {
  encargadoModel.obtenerCarreras().then((carrerasnom) =>
...
router.post("/fMateriaInsertar", function (req, res, next) {
  ...
  encargadoModel
    .insertarMateria(
      ...
      grupom,
      nombremat,
      selectpicker.slice(0, 3),
      selectpicker.slice(-1)
    )
    .then(
      encargadoModel
        .insertarListaMateria(user, siglam, grupom)
        .then((idmateria) => {
...
router.post("/fMateriaActualizar/", function (req, res, next) {
  ...
  encargadoModel
    ...
    .then(
      encargadoModel
        .insertarListaMateria2(user, siglam, grupom, estado)
        .then((idMateria) => {
          res.redirect("/encargado/fMateria");
        })
    )
...
router.get("/fCargaHAgregar", checkNotAuthenticated, function (req, res, next) {
  encargadoModel.obtenerCarreras().then((carreras) =>
    encargadoModel.obtenerSemestre().then((semestres) =>
      encargadoModel.obtenerMaterias().then((materias) =>
        encargadoModel.obtenerDocentes().then((docentes) =>
          encargadoModel.obtenerPeriodo().then((periodos) =>
            encargadoModel.obtenerAulas().then((aulas) =>
              encargadoModel.obtenerDia().then((dias) =>
                res.render("encargado/fCargaHAgregar", {
                  user: req.user.idusuario,
                  carreras: carreras,
                  semestres: semestres,
                  materias: materias,
...
router.post("/fCargaHInsertar", function (req, res, next) {
  ...
  encargadoModel
    .insertarCargaHoraria(
      idcargah,
      descripcion,
      idsemestre,
      carreranums.slice(0, 3),
      carreranums.slice(-1),
      materianums.slice(0, 7),
      materianums.slice(7, 9),
      codigod,
      idperiodo,
      duracion,
      idaula,
    ...
    .then(
      encargadoModel
        .insertarListaCargaHoraria(user, idcargah)
        .then((icargah) => {
...
router.post("/fCargaHActualizar/", function (req, res, next) {
  ...
  encargadoModel
    ...
    .then(
      encargadoModel
        .insertarListaCargaHoraria2(user, idcargah, estado)
        .then((idcargah) => {
          res.redirect("/encargado/fCargaH");
        })
    )
